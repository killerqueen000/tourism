...
import "../calendar.css";
import { Link, useLocation } from "react-router-dom"; // For month navigation
import { MONTHS } from "../../../data/monthUtils";

const Feb = ({ selectedMonth, setSelectedMonth }) => {
      ...
      try {
        ...
        if (data.response && data.response.holidays.length > 0) {
          const holidayList = data.response.holidays
            .filter((holiday) => holiday.date.datetime.month === 2)
            .map((holiday) => ({
              name: holiday.name,
          ...
          console.log("Filtered February Holidays:", holidayList);
          setHolidays(holidayList);
        } else {
          console.warn("No holidays found for February.");
        }
      } catch (error) {
        ...
        <div className="calendar-grid">
          ...
          <div className="dates">
            {Array.from({ length: totalCells }, (_, i) => {
              ...
              const formattedDate = `2025-02-${date
                .toString()
                .padStart(2, "0")}`;
          ...
          {events.map((event, index) => {
            ...
            return (
              <div key={index} className="event-card">
                <h3>{event.title}</h3>
                <p>{dateRange}</p>
                {tags && <small className="event-tags">{tags}</small>}
              </div>
            );
          })}
        </div>
