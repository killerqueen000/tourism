...
const June = ({ selectedMonth, setSelectedMonth }) => {
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (data.response && data.response.holidays.length > 0) {
          const holidayList = data.response.holidays
            .filter((holiday) => holiday.date.datetime.month === 6)
            .map((holiday) => ({
              name: holiday.name,
          ...
          console.log("Filtered June Holidays:", holidayList);
          setHolidays(holidayList);
        } else {
          console.warn("No holidays found for June.");
        }
      } catch (error) {
    ...
    const fetchEvents = async () => {
      try {
        const response = await fetch(
          "http://localhost:5000/api/events?month=June"
        );
        const data = await response.json();
        setEvents(data);
      } catch (error) {
        ...
        <div className="calendar-grid">
          ...
          <div className="dates">
            {Array.from({ length: totalCells }, (_, i) => {
              ...
              const formattedDate = `2025-06-${date
                .toString()
                .padStart(2, "0")}`;
