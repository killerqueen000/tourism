...
const May = ({ selectedMonth, setSelectedMonth }) => {
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (data.response && data.response.holidays.length > 0) {
          const holidayList = data.response.holidays
            .filter((holiday) => holiday.date.datetime.month === 5)
            .map((holiday) => ({
              name: holiday.name,
          ...
          console.log("Filtered May Holidays:", holidayList);
          setHolidays(holidayList);
        } else {
          console.warn("No holidays found for May.");
        }
      } catch (error) {
    ...
    const fetchEvents = async () => {
      try {
        const response = await fetch(
          "http://localhost:5000/api/events?month=May"
        );
        const data = await response.json();
        setEvents(data);
      } catch (error) {
        ...
        <div className="calendar-grid">
          ...
          <div className="dates">
            {Array.from({ length: totalCells }, (_, i) => {
              ...
              const formattedDate = `2025-05-${date
                .toString()
                .padStart(2, "0")}`;
